...
const GroupHeader = ({ groupData, onAddUsers }: GroupHeaderProps) => {
  const [membersWithS3Avatars, setMembersWithS3Avatars] = useState<
    GroupMember[]
  >([]);
  ...
  return (
    <Card
      sx={{
        ...
        mb: { xs: 2, sm: 3 },
        background: (theme) =>
          `linear-gradient(135deg, ${theme.palette.primary.main} 0%, ${theme.palette.secondary.main} 50%, #06b6d4 100%)`,
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          ...
          >
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              >
                {(membersWithS3Avatars.length > 0
                  ? membersWithS3Avatars
                  : groupData.members
                ).map((member) => (
                  <Avatar
