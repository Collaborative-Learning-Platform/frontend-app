...
export function UserStudyPlan() {
  ...
  const [updatingStudyTime, setUpdatingStudyTime] = useState<Set<string>>(new Set());
  ...
  useEffect(() => {
    const fetchStudyPlan = async () => {
      try {
        setIsLoading(true);
        const response = await axiosInstance.get<StudyPlanApiResponse>('/aiservice/getStudyPlan');
        console.log('Full API response:', response.data);
        
        
        if (response.data && response.data.success && response.data.data && response.data.data.plan) {
  ...
  const weeklyData = studyPlan.map(day => ({
    ...
    completionRate: Math.round((day.tasks.filter(task => task.completed).length / day.tasks.length) * 100)
  }));
  ...
  const calculateProgressData = () => {
    ...
    const inProgressDays = studyPlan.filter(day => (day.actualStudyTime || 0) > 0 && day.tasks.some(task => !task.completed)).length;
    
    const completedPercentage = Math.round((completedTasks / totalTasks) * 100);
    const inProgressPercentage = Math.round((inProgressDays / studyPlan.length) * 100);
    ...
    return [
      ...
      { name: 'Pending', value: Math.max(0, pendingPercentage), color: '#e0e0e0ff' }
    ];
  };
  ...
  const handleTaskCompletion = async (dayName: string, taskId: string, completed: boolean) => {
  ...
  // Debounce timer ref
  const studyTimeDebounceRef = useRef<{ [dayName: string]: ReturnType<typeof setTimeout> }>({});

  // Handle actual study time input with debouncing
  const handleActualStudyTime = useCallback((dayName: string, minutes: number) => {
    
    setStudyPlan(prev => prev.map(day => 
      day.day === dayName 
        ? { ...day, actualStudyTime: minutes }
        : day
    ));
    ...
    studyTimeDebounceRef.current[dayName] = setTimeout(async () => {
      try {
        ...
        console.log(`Study time for ${dayName} updated to ${minutes} minutes`);
      } catch (error) {
  ...
  const handleCompleteAllTasks = async (dayName: string, completed: boolean, actualStudyTime: number) => {
    ...
    setStudyPlan(prev => prev.map(d => 
      d.day === dayName 
        ? {
            ...
            ...(completed && (d.actualStudyTime === undefined || d.actualStudyTime === 0) ? 
    ...
    try {
      ...
      if (completed && (day.actualStudyTime === undefined || day.actualStudyTime === 0)) {
        updateData.actualStudyTime = day.totalMinutes;
      }

      await axiosInstance.patch('/aiservice/bulkUpdateTaskCompletion', updateData);
      console.log(`All tasks for ${dayName} ${completed ? 'completed' : 'uncompleted'}`);
    } catch (error) {
      ...
      setStudyPlan(prev => prev.map(d => 
        d.day === dayName 
          ? {
              ...d,
              tasks: d.tasks.map(task => ({ ...task, completed: !completed })),
              // Revert study time if it was auto-set
              ...(completed && day.actualStudyTime === 0 ? { actualStudyTime: 0 } : {})
            }
          : d
      ));
  ...
  const getTaskIcon = (type: string) => {
    switch (type) {
      case 'reading': return <BookIcon />;
      case 'video': return <PlayIcon />;
      case 'quiz': return <QuizIcon />;
      case 'practice': return <ScienceIcon />;
  ...
  const getTaskTypeColor = (type: string) => {
    switch (type) {
      case 'reading': return theme.palette.primary.main;
      case 'video': return theme.palette.error.main;
      case 'quiz': return theme.palette.warning.main;
      case 'practice': return theme.palette.success.main;
        ...
        <Box sx={{ 
          ...
          <Box sx={{ 
            ...
            <Typography variant="body1" color="text.secondary" sx={{ mb: 3, lineHeight: 1.6 }}>
              ...
              Our AI-powered study plan generator can create a customized learning 
              schedule based on your goals, available time, and learning preferences.
            </Typography>
      ...
      <Box sx={{ mb: 4 }}>
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            <TrophyIcon sx={{ color: theme.palette.warning.main, fontSize: 32 }} />
            <Box>
              <Typography variant="h6" fontWeight="bold">
                {studyPlan.flatMap(day => day.tasks).filter(task => task.completed).length > 0 
                  ? "Great progress! Keep up the momentum!" 
                  : "Ready to start your learning journey?"}
              </Typography>
              <Typography variant="body2" color="text.secondary">
                {studyPlan.flatMap(day => day.tasks).filter(task => task.completed).length > 0
                  ? `You've completed ${studyPlan.flatMap(day => day.tasks).filter(task => task.completed).length} out of ${studyPlan.flatMap(day => day.tasks).length} tasks. Stay focused on your goals!`
      ...
      >
        <Paper sx={{ p: 2, textAlign: 'center' }}>
          <Typography variant="h4" fontWeight="bold" color="primary.main">
            {studyPlan.flatMap(day => day.tasks).filter(task => task.completed).length}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            Tasks Completed
          </Typography>
        ...
        <Paper sx={{ p: 2, textAlign: 'center' }}>
          <Typography variant="h4" fontWeight="bold" color="success.main">
            {studyPlan.reduce((acc, day) => acc + (day.actualStudyTime || 0), 0)}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            Minutes Studied
          </Typography>
        ...
        <Paper sx={{ p: 2, textAlign: 'center' }}>
          <Typography variant="h4" fontWeight="bold" color="warning.main">
            {studyPlan.filter(day => (day.actualStudyTime || 0) >= day.totalMinutes).length}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            Daily Goals Met
          </Typography>
        ...
        <Paper sx={{ p: 2, textAlign: 'center' }}>
          <Typography variant="h4" fontWeight="bold" color="info.main">
            {Math.round((studyPlan.flatMap(day => day.tasks).filter(task => task.completed).length / studyPlan.flatMap(day => day.tasks).length) * 100) || 0}%
          </Typography>
          <Typography variant="body2" color="text.secondary">
            Overall Progress
          </Typography>
            ...
            <Paper key={dayPlan.day} sx={{ mb: 2, overflow: 'hidden' }}>
              <Box
                sx={{
                  ...
                  alignItems: 'center',
                  cursor: 'pointer',
                  bgcolor: expandedDay === dayPlan.day ? alpha(theme.palette.primary.main, 0.1) : 'transparent',
              ...
              >
                <Box sx={{ 
                  ...
                  <Typography variant="h6" fontWeight="bold" sx={{ minWidth: 'fit-content' }}>
                    {dayPlan.day}
                  </Typography>
                  <Box sx={{ 
                    display: 'flex', 
                    ...
                    />
                    {dayPlan.actualStudyTime !== undefined && dayPlan.actualStudyTime > 0 && (
                      <Chip 
                        label={`Actual: ${dayPlan.actualStudyTime}min`} 
                        size="small"
                        sx={{ 
                          bgcolor: dayPlan.actualStudyTime >= dayPlan.totalMinutes 
                    ...
                    <Chip 
                      label={`${Math.round((dayPlan.tasks.filter(t => t.completed).length / dayPlan.tasks.length) * 100)}% done`}
                      size="small"
                      sx={{ 
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexShrink: 0 }}>
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      handleCompleteAllTasks(dayPlan.day, true, dayPlan.actualStudyTime || 0);
                    }}
                    disabled={dayPlan.tasks.every(task => task.completed) || dayPlan.tasks.some(task => updatingTasks.has(task.id))}
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      handleCompleteAllTasks(dayPlan.day, false, dayPlan.actualStudyTime || 0);
                    }}
                    disabled={dayPlan.tasks.every(task => !task.completed) || dayPlan.tasks.some(task => updatingTasks.has(task.id))}
                    sx={{ minWidth: 'auto', px: 1, fontSize: '0.75rem' }}
                  >
                  ...
                  <IconButton sx={{ flexShrink: 0 }}>
                    <ExpandMoreIcon 
                      sx={{ 
                        transform: expandedDay === dayPlan.day ? 'rotate(180deg)' : 'rotate(0deg)',
              ...
              <Collapse in={expandedDay === dayPlan.day}>
                ...
                <Box sx={{ p: 2 }}>
                  ...
                  <Box sx={{ mb: 3, p: 2, bgcolor: alpha(theme.palette.info.main, 0.1), borderRadius: 2 }}>
                    ...
                    <Box sx={{ 
                      ...
                      <TextField
                        ...
                        onChange={(e) => handleActualStudyTime(dayPlan.day, parseInt(e.target.value) || 0)}
                        InputProps={{ 
                      ...
                      {dayPlan.actualStudyTime !== undefined && dayPlan.actualStudyTime > 0 && (
                        <Typography 
                          variant="body2" 
                          color={dayPlan.actualStudyTime >= dayPlan.totalMinutes ? 'success.main' : 'warning.main'}
                          sx={{ fontWeight: 500 }}
                        >
                          {dayPlan.actualStudyTime >= dayPlan.totalMinutes ? '✓ Goal achieved!' : 'Keep going!'}
                        </Typography>
                      )}
                    </Box>
                  </Box>
                  ...
                  {dayPlan.tasks.map((task: StudyPlanTask) => (
                    <Box 
                      ...
                      sx={{ 
                        ...
                        p: 2,
                        mb: 1,
                        bgcolor: alpha(getTaskTypeColor(task.type || 'reading'), 0.1),
                        borderRadius: 2,
                        borderLeft: `4px solid ${getTaskTypeColor(task.type || 'reading')}`,
                    ...
                    >
                      <Box sx={{ 
                        ...
                        <Checkbox 
                          ...
                          onChange={(e) => handleTaskCompletion(dayPlan.day, task.id, e.target.checked)}
                          icon={updatingTasks.has(task.id) ? 
                            <CircularProgress size={20} sx={{ color: 'action.disabled' }} /> : 
                            <CheckCircleIcon sx={{ color: 'action.disabled' }} />
                          }
                          checkedIcon={updatingTasks.has(task.id) ? 
                        ...
                        >
                          {task.completed ? <CheckCircleIcon /> : getTaskIcon(task.type || 'reading')}
                        </Avatar>
                        ...
                        <Chip 
                          ...
                          sx={{ 
                            bgcolor: task.completed 
                              ...
                              : alpha(getTaskTypeColor(task.type || 'reading'), 0.2),
